...
import static com.xiaozhameng.sdks.unionpayextend.SDKConstants.*;
 ...
 *
 */
public class SDKUtilExtend {

    private static Logger logger = LoggerFactory.getLogger(SDKUtilExtend.class);
    ...
    public SDKUtilExtend(SDKConfigExtend sdkConfigExtend, CertUtilExtend certUtilExtend) {
        if (sdkConfigExtend == null) {
            logger.error("配置对象com.yeepay.g3.core.bankcommunicate.util.unionpayextend.SDKUtilExtend.sdkConfigExtend 初始化失败");
            throw new RuntimeException("配置对象sdkConfigExtend 初始化失败");
        }
        if (certUtilExtend == null) {
            logger.error("配置对象com.yeepay.g3.core.bankcommunicate.util.unionpayextend.SDKUtilExtend.certUtilExtend 初始化失败");
            throw new RuntimeException("配置对象certUtilExtend 初始化失败");
        }
        this.sdkConfigExtend = sdkConfigExtend;
        this.certUtilExtend = certUtilExtend;
    }
    ...
    public boolean sign(Map<String, String> data, String encoding) {
        ...
        if (SIGNMETHOD_RSA.equals(signMethod) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
            if (VERSION_5_0_0.equals(version) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
                // 设置签名证书序列号
                data.put(SDKConstants.param_certId, certUtilExtend.getSignCertId());
                ...
                try {
                    ...
                    byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
                            certUtilExtend.getSignCertPrivateKey(), signDigest));
            ...
            return signBySecureKey(data, sdkConfigExtend.getSecureKey(), encoding);
    ...
    public boolean signByCertInfo(Map<String, String> data,
        ...
        if (SIGNMETHOD_RSA.equals(signMethod) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
            if (VERSION_5_0_0.equals(version) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
                // 设置签名证书序列号
                data.put(SDKConstants.param_certId, certUtilExtend.getCertIdByKeyStoreMap(certPath, certPwd));
                // 将Map信息转换成key1=value1&key2=value2的形式
                String stringData = coverMap2String(data);
                byte[] byteSign = null;
                String stringSign = null;
                try {
                    ...
                    byte[] signDigest = SecureUtil
                            .sha1X16(stringData, encoding);
                    byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
                            certUtilExtend.getSignCertPrivateKeyByStoreMap(certPath, certPwd), signDigest));
                    stringSign = new String(byteSign);
                    // 设置签名域值
                    data.put(SDKConstants.param_signature, stringSign);
                    return true;
    ...
    public boolean validate(Map<String, String> resData, String encoding) {
        ...
        if (SIGNMETHOD_RSA.equals(signMethod) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
            ...
            if (VERSION_5_0_0.equals(version) || VERSION_1_0_0.equals(version) || VERSION_5_0_1.equals(version)) {
                ...
                try {
                    // 验证签名需要用银联发给商户的公钥证书.
                    return SecureUtil.validateSignBySoft(certUtilExtend
                ...
                X509Certificate x509Cert = certUtilExtend.genCertificateByStr(strCert);
                if (x509Cert == null) {
                    logger.error("convert signPubKeyCert failed");
                    return false;
                }
                // 2.验证证书链
                if (!certUtilExtend.verifyCertificate(x509Cert)) {
                ...
                try {
                    // 验证签名需要用银联发给商户的公钥证书.
                    return SecureUtil.validateSignBySoft256(x509Cert
            ...
            String strBeforeSha256 = stringData
                    + SDKConstants.AMPERSAND
                    + SecureUtil.sha256X16Str(sdkConfigExtend.getSecureKey(), encoding);
            String strAfterSha256 = SecureUtil.sha256X16Str(strBeforeSha256,
                    encoding);
            return stringSign.equals(strAfterSha256);
        } else if (SIGNMETHOD_SM3.equals(signMethod)) {
            ...
            String strBeforeSM3 = stringData
                    + SDKConstants.AMPERSAND
                    + SecureUtil.sm3X16Str(sdkConfigExtend.getSecureKey(), encoding);
            String strAfterSM3 = SecureUtil
                    .sm3X16Str(strBeforeSM3, encoding);
            return stringSign.equals(strAfterSM3);
        }
    ...
    public int getEncryptCert(Map<String, String> resData,
        ...
        if (CERTTYPE_01.equals(certType)) {
            // 更新敏感信息加密公钥
            if (!certUtilExtend.getEncryptCertId().equals(
                    x509Cert.getSerialNumber().toString())) {
                // ID不同时进行本地证书更新操作
                String localCertPath = sdkConfigExtend.getEncryptCertPath();
                ...
                certUtilExtend.resetEncryptCertPublicKey();
                return 1;
            } else {
                return 0;
            }
            ...
            logger.error("unknown cerType:" + certType);
            return -1;
        }
    }
     ...
     * @return
     */
    public String genBackupName(String fileName) {
        if (isEmpty(fileName))
            return "";
    ...
    public byte[] readFileByNIO(String filePath) {
     ...
     * @throws IOException
     */
    public byte[] inflater(final byte[] inputByte) throws IOException {
     ...
     * @throws IOException
     */
    public byte[] deflater(final byte[] inputByte) throws IOException {
        int compressedDataLength = 0;
    ...
    public static Map<String, String> convertResultStringToMap(String result) {
        ...
        try {
            ...
            logger.error("convertResultStringToMap.has.exception:", e);
        }
        return map;
    }
