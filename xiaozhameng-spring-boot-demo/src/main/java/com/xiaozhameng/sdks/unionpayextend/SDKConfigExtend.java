...
/**
 * @Author MaiMangXiaoZhaMeng 操作对象配置信息扩展类，修改之前的静态属性为非静态变量
 * @ClassName SDKConfigExtend
 * @Description acpsdk配置文件acp_sdk.properties配置信息类
 * @date 2016-7-22 下午4:04:55
 *
 */
public class SDKConfigExtend {

	private Logger logger = LoggerFactory.getLogger(SDKConfigExtend.class);
	 ...
	 * 构造器
	 */
	public SDKConfigExtend(String configFileName){
		this.loadPropertiesFromSrc(configFileName);
	}
	...
	private void loadPropertiesFromPath(String rootPath) {
		if (StringUtils.isNotBlank(rootPath)) {
			...
			loadPropertiesFromSrc(FILE_NAME);
		}
	 ...
	 * 从classpath路径下加载配置参数
	 */
	private void loadPropertiesFromSrc(String certFileName) {
		InputStream in = null;
		try {
			in = SDKConfigExtend.class.getClassLoader().getResourceAsStream(certFileName);
			if (null != in) {
				properties = new Properties();
				try {
					properties.load(in);
				...
				logger.error(certFileName + "属性文件未能在classpath指定的目录下 "+SDKConfigExtend.class.getClassLoader().getResource("").getPath()+" 找到!");
				return;
			}
			loadProperties(properties);
		} catch (Exception e) {
